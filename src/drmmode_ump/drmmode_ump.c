...
static int create_custom_gem(int fd, struct armsoc_create_gem *create_gem)
{
	...
	create_gem->handle = ump_ref_drv_allocate(size, UMP_REF_DRV_CONSTRAINT_PHYSICALLY_LINEAR);
	create_gem->name = ump_secure_id_get(create_gem->handle);
	create_gem->pitch = pitch;
	create_gem->size = size;

	return 0;
}

struct drmmode_interface ump_interface = {
